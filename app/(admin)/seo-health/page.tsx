...
// existing seo_pages cache (already populated for every page by DB
// triggers), scores each row client-side via utils/seo/pageScore.ts, and
// gives one place to see every SEO issue, ranked worst-first, with a
// one-click link to the editor that owns each page.
...
import {
  ...
  Gauge, ImageOff, Loader2, RefreshCw, Search, XCircle,
} from 'lucide-react'
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#6b7280] mt-1">
            Every page scored like RankMath/Yoast — title &amp; description quality, content depth, schema, OG images, and internal linking, ranked worst-first.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="admin-card p-4 text-center">
          <p className="text-3xl font-extrabold text-white">{summary.avgScore}</p>
          <p className="text-xs text-[#6b7280] mt-1">Average score</p>
        </div>
        <div className="admin-card p-4 text-center">
          <p className="text-3xl font-extrabold text-white">{summary.total}</p>
          <p className="text-xs text-[#6b7280] mt-1">Pages tracked</p>
        </div>
        <div className="admin-card p-4 text-center">
          <p className="text-3xl font-extrabold text-amber-300">{summary.totalIssues}</p>
          <p className="text-xs text-[#6b7280] mt-1">Total issues (warn + fail)</p>
        </div>
        <div className="admin-card p-4 text-center">
          <p className="text-3xl font-extrabold text-red-400">{summary.noindex}</p>
          <p className="text-xs text-[#6b7280] mt-1">Noindex / inactive</p>
        </div>
      </div>

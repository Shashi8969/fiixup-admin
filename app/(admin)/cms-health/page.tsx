...
// Read-only CMS health dashboard for Supabase audit views.
...
function rowSearchText(row: RowRecord): string {
  return Object.values(row).map(textValue).join(' ').toLowerCase()
}

function issueSeverity(row: RowRecord): 'error' | 'warning' | 'info' {
  const text = rowSearchText(row)
  if (text.includes('error') || text.includes('missing') || text.includes('mismatch') || text.includes('broken')) {
    return 'error'
  }
  if (text.includes('warning') || text.includes('legacy') || text.includes('unused') || text.includes('candidate')) {
    return 'warning'
  }
  return 'info'
}
...
export default function CmsHealthPage() {
  ...
  const summary = useMemo(() => {
    const errorIssues = state.integrityIssues.filter((row) => issueSeverity(row) === 'error')
    const warningIssues = state.integrityIssues.filter((row) => issueSeverity(row) === 'warning')
    const searchRows = state.integrityIssues.map(rowSearchText)
    ...
    return {
      ...
      candidateUnused: searchRows.filter((text) => text.includes('unused') || text.includes('candidate')).length,
      legacyTables: searchRows.filter((text) => text.includes('legacy') || text.includes('testimonial')).length,
    }
  }, [state])
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#6b7280] mt-1">
            Read-only audit dashboard for routes, SEO cache, table roles, legacy tables, and CMS integrity.
          </p>
        </div>

        <button onClick={fetchHealth} disabled={loading} className="admin-btn-secondary self-start md:self-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="admin-card p-4 text-center">
          <AlertTriangle className="w-5 h-5 text-amber-300 mx-auto mb-2" />
          <p className="text-2xl font-extrabold text-white">{summary.totalIssues}</p>
          <p className="text-xs text-[#6b7280] mt-0.5">Integrity rows</p>
        </div>
        <div className="admin-card p-4 text-center">
          <GlobeIcon />
          <p className="text-2xl font-extrabold text-white">{summary.routes}</p>
          <p className="text-xs text-[#6b7280] mt-0.5">Routes checked</p>
        </div>
        <div className="admin-card p-4 text-center">
          <Table2 className="w-5 h-5 text-blue-400 mx-auto mb-2" />
          <p className="text-2xl font-extrabold text-white">{summary.tableRoles}</p>
          <p className="text-xs text-[#6b7280] mt-0.5">Table role rows</p>
        </div>
        <div className="admin-card p-4 text-center">
          <ShieldCheck className="w-5 h-5 text-emerald-400 mx-auto mb-2" />
          <p className="text-2xl font-extrabold text-white">{summary.errorIssues}</p>
          <p className="text-xs text-[#6b7280] mt-0.5">Critical issues</p>
        </div>
      </div>
        ...
        <div className="admin-card p-5">
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            <div className="rounded-xl bg-[#0f1117] border border-[#2a2d3e] p-4">
              ...
              <p className="text-xs text-[#6b7280] mt-1">Candidate unused rows</p>
            </div>
            <div className="rounded-xl bg-[#0f1117] border border-[#2a2d3e] p-4">
              <p className="text-2xl font-extrabold text-white">{summary.legacyTables}</p>
              <p className="text-xs text-[#6b7280] mt-1">Legacy/testimonial rows</p>
            </div>
          </div>
                ...
                <div key={index} className="rounded-xl border border-[#2a2d3e] bg-[#11141d] p-4">
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    ...
                    <p className="text-sm font-semibold text-white">
                      {field(row, ['issue_type', 'issue', 'check_name', 'table_name', 'route_type'], `Issue ${index + 1}`)}
                    </p>
                  </div>
                  <JsonPreview row={row} />
                </div>
            ...
            {state.tableRoles.slice(0, 80).map((row, index) => (
              <div key={index} className="rounded-lg border border-[#2a2d3e] bg-[#0f1117] px-3 py-2">
                ...
                <p className="text-[11px] text-[#6b7280] mt-1 break-words">
                  {field(row, ['role', 'table_role', 'usage', 'status', 'notes'], 'table role')}
                </p>
              </div>
            ))}
...
function GlobeIcon() {
  return (
    <svg className="w-5 h-5 text-blue-400 mx-auto mb-2" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Z" stroke="currentColor" strokeWidth="2" />
      <path d="M3.6 9h16.8M3.6 15h16.8M12 3c2.1 2.2 3.2 5.2 3.2 9s-1.1 6.8-3.2 9c-2.1-2.2-3.2-5.2-3.2-9S9.9 5.2 12 3Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
    </svg>
  )
}

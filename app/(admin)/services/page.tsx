...
import { CreateServiceModal } from '@/components/create/CreateServiceModal'
import { Search, ArrowRight, Package, Plus, Loader2 } from 'lucide-react'
import { clsx } from 'clsx'

const CATS = ['', 'battery', 'bike', 'car', 'mechanic', 'puncture', 'roadside', 'towing']
...
export default function ServicesPage() {
  ...
  const load = async () => {
    setLoading(true)
    let q = sb.from('services').select('id,slug,category,title,short_title,tagline,price,duration').order('category').order('title')
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-[#94a3b8] text-sm mt-1">Global service catalogue — {services.length} services</p>
        </div>
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="relative flex-1 min-w-48">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search services…" className="admin-input pl-9" />
        </div>
      </div>
          ...
          {Object.entries(groups).map(([cat, svcs]) => (
            <div key={cat}>
              ...
              <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
                {svcs.map(s => (
                  <Link key={String(s.id)} href={`/services/${s.slug}`}
      ...
      <CreateServiceModal open={showCreate} onClose={() => setShowCreate(false)} onCreated={load} />
    </div>
  )
}